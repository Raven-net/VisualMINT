class Lander {
  ...
  values() {
    ...
    text(`Treibstoff: ${fuel}`, 600, 70);
    fill(200);
    text(`Leben: ${lifes}`, 600, 90);
  }
  applyForce(force) {
    this.acc.add(force);
  }
  ...
  edges() {
    if (collidePointPoly(this.bottom.x, this.bottom.y, ground.leftGround)) {
      this.pos.y = 150;
      this.pos.x = 400;
      this.vel.set(0, 0);
      lostLife = true;
    ...
    if (collidePointPoly(this.bottom.x, this.bottom.y, ground.rightGround)) {
      this.pos.y = 150;
      this.pos.x = 400;
      this.vel.set(0, 0);
      lostLife = true;
